...
class RalphConfig:
    ...
    dropout: float = 0.0          # residual dropout on attn/ffn sublayer outputs
    logit_z_coef: float = 0.0     # z-loss: += coef * mean(logsumexp(logits)^2)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rope_cache: torch.Tensor) -> torch.Tensor:
        x = x + self.drop(self.attn_norm(self.attn(x, rope_cache)))
        x = x + self.drop(self.ffn_norm(self.ffn(x)))
        return x
...
class RalphBase(nn.Module):
    ...
    def __init__(self, cfg: RalphConfig):
        ...
        if cfg.tie_embeddings:
            ...
            nn.init.zeros_(self.lm_head.weight)  # #1317: zero-init => identity logits at step 0
