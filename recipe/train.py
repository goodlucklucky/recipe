...
class TrainConfig:
    ...
    val_frac: float = 0.0
    val_points: int = 0
    # LAWA (latest weight averaging): average the model weights over the last
    # `lawa_frac` of the run (snapshotted at val points), evaluate the average on
    # the held-out split, and keep it ONLY if it beats the held-out-min checkpoint.
    # Training-only, no new state-dict keys (op4-safe). 0 = off.
    lawa_frac: float = 0.0
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    # Epoch-permutation, no-replacement sampling over the TRAIN region
    # [0, _val_start): the k-th sample is the perm[k mod n_win]-th non-overlapping
    # (seq_len+1) window, reshuffled each epoch with a (seed, epoch)-keyed
    # permutation — every train token seen once per epoch (uniform coverage),
    # mirroring data/dataset.py's sampler while keeping the val tail untouched.
    # Deterministic: a pure function of (manifest, seed, seq_len, k).
    ...
    # LAWA: accumulate a running SUM of the weights over the last lawa_frac of steps.
    ...
    for step in range(cfg.total_steps):
        ...
        if _val_interval and step > 0 and (step % _val_interval == 0 or step == cfg.total_steps - 1):
            ...
            # LAWA: accumulate this snapshot into the running weight sum (tail window only).
    ...
    # LAWA: average the tail snapshots, evaluate on the held-out split, and keep it
    # ONLY if it beats the held-out-minimum checkpoint (so LAWA can only help).
